...
namespace CommonViewer
{
    public partial class ButtonEx : Button
    {
        ...
        {
            get { return _radius; }
            set { if (value > 0) _radius = value; this.Refresh(); }
        }
        ...
        {
            get { 
                return this.Enabled ? _baseColor : Color.Silver; 
            }
            set
            {
                _baseColor = value;
            ...
            if (this.Text != "")
            {
                ...
                switch (_imagelocation)
                {
                    case e_imagelocation.Left:
                        if (this.Title != "")
                        {
                            gr.DrawString(this.Title, fontb, new SolidBrush(_forecolor), new PointF(offsetx * 2 + imagewidth , this.Font.Size / 2));
                            gr.DrawString(s1, this.Font, new SolidBrush(_forecolor), new PointF(offsetx * 2 + imagewidth , 2 * this.Font.Size + 1));
                            gr.DrawString(s2, this.Font, new SolidBrush(_forecolor), new PointF(offsetx * 2 + imagewidth , 3 * this.Font.Size + 4));
                        }
                        else
                        {
                            gr.DrawString(s1, this.Font, new SolidBrush(_forecolor), new PointF(offsetx * 2 + imagewidth , this.Height / 2 - this.Font.Size/2 - 1 ));
                        }

                        break;
                    case e_imagelocation.Right:
                        gr.DrawString(this.Title, fontb, new SolidBrush(_forecolor), new PointF(offsetx, this.Height / 2 - this.Font.Size/2 + 1 - extraoffset));
                        gr.DrawString(this.Text, this.Font, new SolidBrush(_forecolor), new PointF(offsetx, extraoffset + this.Height / 2 - this.Font.Size / 2 - 1));
                        break;
                    case e_imagelocation.Top:
                        gr.DrawString(this.Text, this.Font, new SolidBrush(_forecolor), new PointF(this.Width / 2 - textwidth / 2 - 1, offsety  * 2 + imageheight));
                        break;
                    case e_imagelocation.Bottom:
        ...
        public void UpdateLeave()
        {
            if (_keeppress == false | (_keeppress == true & _ispressed == false))
            {
                ...
                if (i_factor == 0)
                {
                    R0 = ColorBase.R; G0 = ColorBase.G; B0 = ColorBase.B;
                    this.Refresh();
                }
                else
                {
        ...
        protected override void OnMouseUp(MouseEventArgs mevent)
        {
            ...
            else if (_splitbutton == e_splitbutton.No & _arrow != e_arrow.None)
            {
                if (this.ContextMenuStrip != null)
                    this.ContextMenuStrip.Show(this, 0, this.Height);
            }
            ...
            else
            {
                ...
                if (_keeppress)
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (typeof(ButtonEx) == _control.GetType() & _control.Name != this.Name)
                            {
                                ((ButtonEx)(_control))._ispressed = false;
                                ((ButtonEx)(_control)).UpdateLeave();
                            }
                        }
                    }
                    catch { }
        ...
        private Color _onColorBack;
        private Color _baseColorBack;
        public void ShiftButtonDownColorWithButtonOn(bool shift)
        {
            if (shift)
            {
                _onColorBack = ColorOn;
                _baseColorBack = ColorBase;
                ColorBase = _pressColor;
            }
            else
            {
                if (_baseColorBack == null || _baseColorBack.ToArgb() == 0)
                {
                    _onColorBack = ColorOn;
                    _baseColorBack = ColorBase;
                }
                ...
                {
                    ColorOn = _onColorBack;
                    ColorBase = _baseColorBack;
                }
            } 
        }
        #endregion
