...
namespace LSShipMis_Land.Common.Classes
{
    /// <summary>
    /// 可引用所有无参数方法的委托.
    /// </summary>
    ...
    public class CommonOpt
    {
            ...
            foreach (DataRow dataRow in dtbAdded.Rows)
            {
                foreach (DataColumn dataColumn in dtbAdded.Columns)
                {
                    ...
                    else if (dataColumn.DataType.Name.Equals("DateTime"))
                    {
                        ...
                        {
                            sInsertVal += EnvironmentParm.DbToDate(dataRow[dataColumn].ToString()) + ",";
                        }
                    }
                    //其它类型数据.
                    else
            ...
            foreach (DataRow dataRow in dtb.Rows)
            {
                for (int iCount = 1; iCount < dtb.Columns.Count; iCount++)
                {
                    ...
                    else if (dataColumn.DataType.Name.Equals("DateTime"))
                    {
                        ...
                        {
                            sUpdateSql += dataColumn.ColumnName + " = " + EnvironmentParm.DbToDate(dataRow[dataColumn].ToString()) + ",";
                        }
                    }
                    //其它型数据.
                ...
                {
                    if (EnvironmentParm.DbmsUsed == DBMSUsed.SqlServer)
                    {
                        sUpdateSql += "UpdateTime = getdate(),";
                    }
                    else
                    {
                        sUpdateSql += "UpdateTime = sysdate,";
                    }
                }
                //组装好更新当前行的Update语句.
        ...
        public void SetComboBoxValue(ComboBox cbo, DataTable dtb)
        {
            ...
            {
                valueFld = dtb.Columns[0].ColumnName;
                nameFld = dtb.Columns[1].ColumnName;
            }
        ...
        /// <param name="IsReadOnly">true，设置为只读；false，取消只读</param>
        /// <returns></returns>
        public bool SetFileAttribute(string filePath, bool IsReadOnly)
        {
            try
            {
                ...
                fileInfo.IsReadOnly = IsReadOnly;//设置文件是否只读.
                return true;
            }
            catch (IOException)
            {
        ...
        public void DgvSearch(DataGridView dgv, string searchCol, string searchValue, ref int currentRow)
        {
            int find = 0;
            if (dgv.Rows.Count < currentRow) return;
            ...
            try
            {
                for (int i = currentRow; i < dgv.Rows.Count; i++)
                {
                    ...
                    {
                        str += dgv[curSearchName.Trim(), i].Value.ToString() + ";";
                    }
                ...
                {
                    currentRow = 0;
                    MessageBoxEx.Show("没有查找的项了");
                }
                else
                {
                    dgv.CurrentCell = dgv[searchCols[foundCol], currentRow - 1];
                }
        ...
        public void DgvSearch(DataGridView dgv, string searchCol, string searchValue, bool showMsg, ref int currentRow)
        {
            int find = 0;
            if (dgv.Rows.Count < currentRow) return;
            ...
            try
            {
                ...
                {
                    currentRow = 0;
                    if (showMsg) MessageBoxEx.Show("没有查找的项了");
                }
                else
                {
                    dgv.CurrentCell = dgv[searchCols[foundCol], currentRow - 1];
        ...
        /// <summary>
        /// 判断在网格dgv的colname列中sValue值是否已经存在，如果存在则返回true
        /// </summary>
        /// <param name="dgv">DataGridView网格控件</param>
        /// <param name="colname">要校验的列名称</param>
        /// <param name="colname">要校验的值</param>
        /// <returns>返回bool值</returns>
        public bool IsExistInDgv(DataGridView dgv, string colname, string sValue)
        {
            bool blnResult = false;
        ...
        /// <summary>
        /// 判断在网格dgv的colname1列中sValue1值和colname2列中sValue2值是否同时存在，如果存在则返回true
        /// </summary>
        /// <param name="dgv">DataGridView网格控件</param>
        /// <param name="colname1">要校验的列名称1</param>
        /// <param name="sValue1">要校验的值1</param>
        /// <param name="colname2">要校验的列名称2</param>
        /// <param name="sValue2">要校验的值2</param>
        /// <returns>返回bool值</returns>
        public bool IsExistInDgv(DataGridView dgv, string colname1, string sValue1,
                                 string colname2, string sValue2)
        {
            bool blnResult = false;
            ...
            foreach (DataGridViewRow dgvRow in dgv.Rows)
            {
                ...
                if (sCurCell1.Equals(sValue1) && sCurCell2.Equals(sValue2))
                {
                    return true;
                }
            }
        ...
        /// <param name="colname">要校验的列名称</param>
        /// <returns>返回bool值</returns>
        public bool HasRepliVal(DataGridView dgv, string colname)
        {
            bool blnResult = false;
            ...
            {
                string sCurCell = dgvRow.Cells[colname].Value.ToString();
                if (this.HasRepliVal(dgv, colname, sCurCell))
                {
                    return true;
                }
            }
        ...
        /// <returns>返回bool值</returns>
        public bool HasRepliVal(DataGridView dgv, string colname1, string colname2, int diff)
        {
            bool blnResult = false;
        ...
        /// <returns>返回bool值</returns>
        public bool HasRepliVal(DataGridView dgv, string colname, string curVal)
        {
        ...
        /// <returns>返回bool值</returns>
        public bool HasRepliVal(DataGridView dgv, string colname1, string colname2, string curVal)
        {
            int replicount = 0;     //值curVal在列colname中重复的次数.
            bool blnResult = false;
        ...
        /// <summary>
        /// 判断网格dgv的列colname中的所有值是否都是日期型的，如果不是则返回false
        /// </summary>
        /// <param name="dgv">DataGridView网格控件</param>
        /// <param name="colname">要校验的列名称</param>
        /// <returns>返回bool值</returns>
        public bool IsDateTime(DataGridView dgv, string colname)
        {
            bool blnResult = true;
            ...
            {
                string sCurCell = dgvRow.Cells[colname].Value.ToString();
                if (DateTime.TryParse(sCurCell, out dateValidate) == false)
                {
                    blnResult = false;
                    break;
                }
        ...
        /// <param name="colname">要校验的列名称</param>
        /// <returns>返回bool值</returns>
        public bool IsNumeric(DataGridView dgv, string colname)
        {
            bool blnResult = true;
        ...
        /// <summary>
        /// 设置指定网格中的指定列为只读（主要用于备件和物料中非手工添加的备件与物料信息）.
        /// </summary>
        /// <param name="dgv">DataGridView网格控件</param>
        /// <param name="sId">主键字段</param>
        /// <param name="sCol">包含要设置只读属性的列名称的数组</param>
        public void setDgvCellReadOnly(DataGridView dgv, string sId, string[] sCol)
        {
            foreach (DataGridViewRow dgvRow in dgv.Rows)
            {
                string curId = dgvRow.Cells[sId].Value.ToString();
        ...
        /// <param name="e"></param>
        public void PictureFormat(Object sender, ConvertEventArgs e)
        {
            ...
            catch (NullReferenceException) { e.Value = null; }
            catch (ArgumentException) { e.Value = null; }
            catch (InvalidCastException) { e.Value = null; }
            finally
            {
                ms.Close();
            }
