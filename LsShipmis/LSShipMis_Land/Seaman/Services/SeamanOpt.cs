...
namespace LSShipMis_Land.Seaman.Services
{
    ...
    public class SeamanOpt
    {
        ...
        /// 定义一个公共类CommonOpt对象.
        /// </summary>
        private static CommonOpt commonOpt = new CommonOpt();
                                                        ...
                                                        string shipId, string voytimesId, out string err)
        {
            ...
            sUpdateSql += "set downshiptime= " + LSShipMis_Land.Common.EnvironmentParm.DbToDate(dtb.Rows[0]["senddate"].ToString());
            ...
            sInsertSql += "'" + voytimesId + "'," + LSShipMis_Land.Common.EnvironmentParm.DbToDate(dtb.Rows[0]["senddate"].ToString()) + ",";
        ...
        public static void SaveSeamanInfo(DataTable dtb, string fileName, string shipUserId, out string err)
        {
            ...
            //保存照片大对象数据.
            if (err.Equals("") && fileName.Length>0)
            {
                string sUpdateBlob = "";
                string sInsertBlob = "";
                ...
                {
                    sInsertBlob = "update t_ship_user set photo = 'aaa' where shipuserId='" + shipUserId + "'";
                    sUpdateBlob = "select photo from t_ship_user where shipuserId='" + shipUserId + "' for update";
                    dbAccess.WriteBlobToDbWithIns(sInsertBlob, sUpdateBlob, fileName, "", out err);
                }
                else
                {
