...
namespace LSShipMis_Land.Seaman.Services
{
    /// <summary>
    /// 在船人员业务信息.
    /// </summary>
    public class SeamanService
    {
        ...
        public static DataTable GetSeamanInfo()
        {
            ...
            sSql += "left join t_base_headship b on a.ship_headship_id=b.ship_headship_id ";
            sSql += "left join T_DEPARTMENT c on a.unit_id=c.DEPARTMENTID ";
            sSql += "left join t_country d on a.countryid=d.countryid where a.username != '超级用户'";
            sSql += "order by a.username";//按创建时间排序.
        ...
        public static DataTable GetInShip(string shipId, string voytimesId, string inshipType, string upTime, string downTime)
        {
            ...
            {
                if (LSShipMis_Land.Common.EnvironmentParm.DbmsUsed == DBMSUsed.Oracle)
                {
                    //查询非在船人员的历史数据（上船时间与下船时间在指定日期范围内的数据）.
                    sWhere += "and a.ship_id = '" + shipId + "' and voytimesid = '" + voytimesId + "' and (a.upshiptime>=to_date('" + upTime + "','yyyy-MM-dd hh24:mi:ss') ";
                    sWhere += "and (a.downshiptime is not null and a.downshiptime<=to_date('" + downTime + "','yyyy-MM-dd hh24:mi:ss'))) ";
                }
                else if (LSShipMis_Land.Common.EnvironmentParm.DbmsUsed == DBMSUsed.SqlServer)
                {
                    //查询非在船人员的历史数据（上船时间与下船时间在指定日期范围内的数据）.
        ...
        public static DataTable GetInShip(string inshipType, string upTime, string downTime)
        {
            ...
            else
            {
                ...
                {
                    //查询非在船人员的历史数据（上船时间与下船时间在指定日期范围内的数据）.
                    sWhere += "and (upshiptime>=to_date('" + upTime + "','yyyy-MM-dd hh24:mi:ss') ";
                    sWhere += "and (downshiptime is not null and downshiptime<=to_date('" + downTime + "','yyyy-MM-dd hh24:mi:ss'))) ";
                }
                else if (LSShipMis_Land.Common.EnvironmentParm.DbmsUsed == DBMSUsed.SqlServer)
                {
                    //查询非在船人员的历史数据（上船时间与下船时间在指定日期范围内的数据）.
        ...
        public static DataTable GetWkShip(string shipId,string startdate, string enddate)
        {
            ...
            if (LSShipMis_Land.Common.EnvironmentParm.DbmsUsed == DBMSUsed.Oracle)
            {
                sWhere += "and ship_Id = '" + shipId + "' and senddate between to_date('";
                sWhere += startdate + "','yyyy-MM-dd hh24:mi:ss') ";
                sWhere += "and to_date('" + enddate + "','yyyy-MM-dd hh24:mi:ss')";
            }
            else if (LSShipMis_Land.Common.EnvironmentParm.DbmsUsed == DBMSUsed.SqlServer)
            {
