...
namespace StorageManage.Services
{
    public partial class SparePurchaseOrderService
    {
        ...
        public bool GetInfo(string PURCHASE_ORDERID, string SHIP_ID, string STATE, out DataTable dt, out string err)
        {
            ...
            sb.AppendLine(",dbo.F_Get_Comp_Full_name(spo.COMPONENT_UNIT_ID,1) COMP_FULL_NAME");
            ...
            sb.AppendLine(" from T_SPARE_PURCHASE_ORDER spo");
        ...
        /// </summary>
        /// <returns>T_SPARE_PURCHASE_ORDER DataTable</returns>
        public bool GetInfo(SparePurchaseOrderQueryParameter qp, out DataTable dt, out string err)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("select	");
            sb.AppendLine("spo.PURCHASE_ORDERID");
            ...
            if (qp.IsRemoveReference)
                sb.AppendLine(" and spo.PURCHASE_ORDER_CODE not in (select ORDER_CODE from T_SPARE_DEPOT_OPERATION_DETAIL where ORDER_CODE is not null group by ORDER_CODE)");
            sb.AppendLine(" order by spo.PURCHASE_ORDER_DATE desc");
            return (dbAccess.GetDataTable(sb.ToString(), out dt, out err));
        }
        /// <summary>
        ...
        public string GetPurchaseOrderCode(string shipId)
        {
            ...
            sSql += "select cast(right(PURCHASE_ORDER_CODE,3) as int) as PURCHASE_ORDER_CODE from T_SPARE_PURCHASE_ORDER where ship_id = '" + shipId + "') a";
        ...
        public string saveUnit(SparePurchaseOrder unit, int operationType)
        {
            if (operationType == 2)
            {
        ...
        public CheckObj GetSparePurchaseOrderCheckObjByOrderId(string purchaseOrderId)
        {
            string err;
            DataTable dt;
            sql = string.Format(@"select top 5 t1.PURCHASE_ORDERID as businessid ,
 t3.SHIP_NAME  +   '(' + t1.PURCHASE_ORDER_CODE+ ') 备件采购' as businessName,
...
t2.Spare_NAME +isnull('('+t2.partnumber+') ',' ')+  cast(CAST( t2.PURCHASECOUNT as numeric(18,0)) as varchar(100)) + isnull( +t5.UNIT_NAME ,'') + isnull( t2.REMARK ,'') as businessItemInfo
from T_Spare_PURCHASE_ORDER t1 inner join T_Spare_PURCHASE_ORDER_DETAIL t2 on t1.PURCHASE_ORDERID = t2.PURCHASE_ORDERID
inner join T_SHIP t3 on t1.SHIP_ID = t3.SHIP_ID
inner join T_MANUFACTURER t4 on t1.SUPPLIER_ID = t4.MANUFACTURER_ID
left join T_Spare t5 on t2.Spare_ID = t5.Spare_ID
where t1.PURCHASE_ORDERID = '{0}'", dbOperation.ReplaceSqlKeyStr(purchaseOrderId));
    ...
    public struct SparePurchaseOrderQueryParameter
    {
