...
{
    public class SpareNotVoucherAlert : CommonOperation.Interfaces.IRemind
    {
        private static string message = "存在未填写的备件入库凭证";
        ...
        {
            FrmSpareVoucher frm = FrmSpareVoucher.Instance;
        ...
        public static IRemind GetAlarmObject()
        {
            string err;
            DataTable dtb;
            SparePurchaseOrderService.Instance.GetInfo(null, "", "8", out dtb, out err);
            if (dtb != null && dtb.Rows.Count > 0)
                return (IRemind)new SpareNotVoucherAlert();
            return null;
        }
    }
}
