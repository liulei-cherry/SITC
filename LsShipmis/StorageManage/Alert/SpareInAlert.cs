/****************************************************************************************************
 * Copyright (C) 2007 大连陆海科技有限公司 版权所有 * 文 件 名：SpareInAlert.cs
 * 创 建 人：李景育 * 创建时间：2008-04-20
 * 标    题：船舶备件入库报警接口模块
 * 功能描述：实现船舶备件入库报警接口模块的业务类 * 修 改 人： 
 * 修改时间： * 修改内容： ****************************************************************************************************/
using System;
...
namespace StorageManage.Alert
{
    ...
    /// 备件入库报警业务类.
    /// </summary>
    public class SpareInAlert : CommonOperation.Interfaces.IRemind
    {
        /// <summary>
        /// 定义数据操作层对象.
        /// </summary>
        ...
        public static IRemind GetAlarmObject(string userId)
        {
            ...
            sb.AppendLine("where IN_OR_OUT=1 and ISCOMPLETE=1");
            sb.AppendLine(sSql);
            dbAccess.GetDataTable(sb.ToString(), out dtb, out err);
            if (dtb != null && dtb.Rows.Count > 0)
            {
                return (IRemind)new SpareInAlert();
            }
            else
            {
                return null;
