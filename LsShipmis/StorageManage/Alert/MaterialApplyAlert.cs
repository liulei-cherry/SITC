...
namespace StorageManage.Alert
{
    ...
    /// 物资申请报警业务类.
    /// </summary>
    public class MaterialApplyAlert : CommonOperation.Interfaces.IRemind
    {
        ...
        public static IRemind GetAlarmObject(int functionCode)
        {
            ...
            sb.AppendLine(" from T_Material_PURCHASE_APPLY");
            if (CommonOperation.ConstParameter.IsLandVersion)
                sb.AppendLine(" inner join T_USER_SHIP us on us.ship_id=T_Material_PURCHASE_APPLY.ship_id and us.userid = '" + CommonOperation.ConstParameter.UserId + "'");
            ...
            if (dbAccess.GetDataTable(sb.ToString(), out dtb, out err))
            {
                if (dtb != null && dtb.Rows.Count > 0)
                    return (IRemind)new MaterialApplyAlert();
            }
            return null;
        }
    }
