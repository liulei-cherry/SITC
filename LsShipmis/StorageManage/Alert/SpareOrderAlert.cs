...
namespace StorageManage.Alert
{
    ...
    /// 备件订单报警业务类.
    /// </summary>
    public class SpareOrderAlert : CommonOperation.Interfaces.IRemind
    {
        ...
        public static IRemind GetAlarmObject(int functionCode)
        {
            ...
            sb.AppendLine(" from T_SPARE_PURCHASE_ORDER");
            if (CommonOperation.ConstParameter.IsLandVersion)
                sb.AppendLine(" inner join T_USER_SHIP us on us.ship_id=T_SPARE_PURCHASE_ORDER.ship_id and us.userid = '" + CommonOperation.ConstParameter.UserId + "'");
            ...
            if (dbAccess.GetDataTable(sb.ToString(), out dtb, out err))
            {
                if (dtb != null && dtb.Rows.Count > 0)
                    return (IRemind)new SpareOrderAlert();
            }
            return null;
        }
    }
